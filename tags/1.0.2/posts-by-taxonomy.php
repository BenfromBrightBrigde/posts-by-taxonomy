...
function bbwd_PBTD_PostDisplay($bbwdAtts = ''){
	$bbwdDisPosts = 'post';
	$bbwdDisTax = '';
// 	$bbwdFeatFallback = the_custom_logo();
...
// 		var_dump($bbwdPostsToDisplay->found_posts);
	...
	if($bbwdTesting){
		$bbwdPostType = $bbwdAtts['post-type'] ? "Post type selected = ".$bbwdAtts['post-type'] : "No post type selected" ;
		$bbwdTaxSelected = $bbwdAtts['taxonomy'] ? "Taxonomy selected = ".$bbwdAtts['taxonomy'] : "No taxonomy selected" ;
		$bbwdPostPerSec = $bbwdAtts['post-per-section'] ? $bbwdAtts['post-per-section']." posts per section" : "No limit set on post per section" ;
		$bbwdFallBackUrl = $bbwdAtts['fallback-image'] ? "Fallback url = ".$bbwdAtts['fallback-image'] : "No fallback image selected" ;
		$bbwdSandFID = $bbwdAtts['s-and-f'] ? "Search and Filter ID = ".$bbwdAtts['s-and-f'] : "No Search and Filter id selected" ;
		$bbwdRawExludes = $bbwdAtts['exclude'] ? $bbwdAtts['exclude']." are excluded" : "No excluded taxonomy items" ;
		...
		echo "<p>".esc_html($testFPInt)." Total Articles</p>";
		echo "<p>".esc_html($testCPInt)." Total Articles Currently Displayed</p>";
	}
	}
